...
// ============================================================================
// kstrie_iter -- bidirectional iterator and traversal for kstrie
//
// Non-friend. Accesses kstrie only through public API:
//   get_root(), find(), contains(), insert(), erase(), size(), empty()
//
// All traversal walks the node tree directly via public static methods
// on hdr_type, compact_type, bitmask_type, slots_type.
// ============================================================================
...
public:
    ...
    class const_iterator {
        const kstrie_iter* iter_ = nullptr;
        std::string        key_;
        VALUE              value_{};
        bool               at_end_ = true;

        const_iterator(const kstrie_iter* it, std::string k, const VALUE& v)
            : iter_(it), key_(std::move(k)), value_(v), at_end_(false) {}

        static const_iterator make_end(const kstrie_iter* it) {
            const_iterator ci;
            ci.iter_   = it;
            ci.at_end_ = true;
            return ci;
    ...
    public:
        ...
        const_iterator& operator--() {
            if (at_end_) {
                auto [k, v] = iter_->iter_max(iter_->trie_.get_root());
                if (v) { key_ = std::move(k); value_ = *v; at_end_ = false; }
            } else {
                auto [k, v] = iter_->iter_prev(key_);
...
private:
    ...
    // ------------------------------------------------------------------
    // find_min_impl_tail -- min from node after skip already handled
    // ------------------------------------------------------------------
    ...
    iter_next(const std::string& current) const {
        ...
        std::string result;
        const VALUE* val = find_next_impl(trie_.get_root(), mapped, len,
                                          0, result);
        delete[] heap_buf;
        return {std::move(result), val};
    }
    ...
    iter_prev(const std::string& current) const {
        ...
        std::string result;
        const VALUE* val = find_prev_impl(trie_.get_root(), mapped, len,
                                          0, result);
        delete[] heap_buf;
        return {std::move(result), val};
    }
    ...
    iter_lower_bound(std::string_view key) const {
        ...
        std::string result;
        const VALUE* val = find_ge_impl(trie_.get_root(), mapped, len,
                                        0, result);
        delete[] heap_buf;
        return {std::move(result), val};
    }
    ...
    prefix_result iter_prefix_bounds(std::string_view pfx) const {
        ...
        const uint64_t* node = trie_.get_root();
        uint32_t consumed = 0;
...
public:
    ...
    explicit kstrie_iter(const trie_type& t) : trie_(t) {}
    ...
    const_iterator begin() const {
        auto [k, v] = iter_min(trie_.get_root());
        if (!v) return end();
    ...
    const_iterator find(std::string_view key) const {
        const VALUE* v = trie_.find(key);
        if (!v) return end();
        return {this, std::string(key), *v};
    }

    // ------------------------------------------------------------------
    // Mutating operations (delegate to trie)
    // ------------------------------------------------------------------

    const_iterator erase(const_iterator pos, trie_type& trie) {
    ...
    std::pair<const_iterator, bool> emplace(trie_type& trie,
                                            std::string_view key,
                                            Args&&... args) {
        VALUE v(std::forward<Args>(args)...);
        bool inserted = trie.insert(key, v);
        auto [k, vp] = iter_lower_bound(key);
        return {const_iterator{this, std::move(k), *vp}, inserted};
    }

    template <typename... Args>
    std::pair<const_iterator, bool> try_emplace(trie_type& trie,
                                                std::string_view key,
                                                Args&&... args) {
        const VALUE* existing = trie.find(key);
        if (existing)
    ...
    bool operator==(const kstrie_iter& o) const {
        if (trie_.size() != o.trie_.size()) return false;
    ...
    bool operator!=(const kstrie_iter& o) const { return !(*this == o); }
};
